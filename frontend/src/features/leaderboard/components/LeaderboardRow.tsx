      ...
      >
        ...
        <span
          className={cn(
            ...
            isCurrentUser ? 'text-white font-medium' : 'text-zinc-200'
          )}
        >
          {displayName}
          {isCurrentUser && (
        ...
        {exactCount !== undefined && (
          <div className="flex items-center gap-0.5 flex-shrink-0 w-9 justify-end">
            <Star
              size={11}
              className={
                exactCount > 0 ? 'text-amber-400 fill-amber-400' : 'text-zinc-700 fill-zinc-700'
              }
            />
            <span
              className={cn(
                'text-xs tabular-nums',
                exactCount > 0 ? 'text-zinc-400' : 'text-zinc-700'
              )}
            >
              {exactCount}
            </span>
