...
export function tierToPoints(tier: PredictionTier, config: ScoringConfiguration): number {
  const m = config.phase_multiplier
  switch (tier) {
    case 'exact':
      return config.exact_score_points * m
    case 'partial_correct_winner':
      return config.partial_correct_winner_points * m
    case 'correct_winner':
      return config.correct_result_points * m
    case 'partial_wrong':
      return config.partial_wrong_winner_points * m
    case 'miss':
      return 0
  }
}
