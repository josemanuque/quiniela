...
export const queryKeys = {
  ...
  matches: (roundId?: string) => (roundId ? ['matches', roundId] : (['matches'] as const)),
  matchday: (competitionId: string, md: number) =>
    ['matches', 'matchday', competitionId, md] as const,
  ...
  leaderboard: (scope: string, type: 'confirmed' | 'projected') =>
    ['leaderboard', scope, type] as const,
  scoringConfig: (competitionId: string) => ['scoring', competitionId] as const,
  groupTrajectory: (groupId: string, g: string) => ['group-trajectory', groupId, g] as const,
} as const
